...
public class EmployeeReport extends JInternalFrame {
	...
	public EmployeeReport() {
		...
		try {
			...
			rs = st.executeQuery("select COUNT(*) from employee_master");
			rs.next();
			Object rowData[][] = new Object[rs.getInt(1)][8];
			Object columnNames[] = { "Sr.No", "Name", "Address", "Contact No", "Email", "Date Of Birth", "Salary", "Join Date" };
			int i = 0, j = 0;
			...
			while (rs.next()) {
				...
				rowData[j][i++] = rs.getString(7);
				rowData[j++][i++] = rs.getString(9);
			}
